...
void main(void)
{
    ...
    vec3 rgbSampleNeg = texture(u_colorTexture, TexCoord + samplingDirection * (1.0/3.0 - 0.5)).rgb;
    vec3 rgbSamplePos = texture(u_colorTexture, TexCoord + samplingDirection * (2.0/3.0 - 0.5)).rgb;

    vec3 rgbTwoTab = (rgbSamplePos + rgbSampleNeg) * 0.5;

    // Outer samples on the tab.
    vec3 rgbSampleNegOuter = texture(u_colorTexture, TexCoord + samplingDirection * (0.0/3.0 - 0.5)).rgb;
    vec3 rgbSamplePosOuter = texture(u_colorTexture, TexCoord + samplingDirection * (3.0/3.0 - 0.5)).rgb;
